---
title: "An introduction to data visualization using ggplot2, part 1"
date: 2017-09-05
output: 
  html_document: 
    toc: true
...
This file introduces basic concepts of plotting using the ggplot2 package, which implements the grammar of graphics. This is a coherent system for describing and building graphs in R.
...
If you get an error message complaining about a missing package, you'll need to install it. Uncomment the code in the following chunk and run it:
...
You will work with two datasets that are built-in to ggplot package - `mpg` and `diamonds` to practice the grammar of graphics concepts.
...
# note: you can also click the mpg variable in the environment to view the dataset in a table

# You can see description of the dataset by running:
...
**Note:** Make sure to always put + at the end of the line, not at the start.
...
For points, aesthetics include size, shape, color, alpha (transparency). Aesthetics available for other geometrical objects are listed [here](http://r4ds.had.co.nz/data-visualisation.html#geometric-objects).

You can also check built-in help: `?geom_point`.

For each value of the variable, an unique level of aesthetic is assigned (e.g. unique color). This assignment is controlled by a process known as *scaling*. The legend describing mappings between variables and levels is automatically included.
...
Recreate the visualization shown in the picture below using the `diamonds` dataset.
...
Recreate the visualization shown  below using the `diamonds` dataset.
See `?diamonds` if necessary
