...
1. Download this Rmarkdown file to your computer. Note, that it requires a data file - `students-mat.xlsx` to reside in the same folder.
...
3. When finished all questions, run all chunks (`Run/Run All` or `Ctrl-Alt-R`). Then press `Preview` button on the toolbar in RStudio. This should create a `alco_eda.nb.html` file in the same folder as `alco_eda.Rmd` (this file).

4. Send me the resulting `alco_eda.nb.html` file. 
...
Put your code to load libraries and the data in the following code chunk. You'll need the `tidyverse` and the `scales` packages for this assignment. Use `read_excel()` to read-in the data file.
