...
A slope coefficient in a logistic regression model shows the change in logit, i.e. the log-odds for unit change of the predictor. In our model all predictors are dummy variables, so the coefficients show the difference for the corresponding groups of passengers.

Since we've included gender, class and the interaction of the two into the model, now we can compare the survival probability for two passengers of the same gender travelling in the two classes. We can also compare the survival chances of a man and a woman travelling in the same class.

Our baseline category in terms of class is the 1st class passenger. A 2nd class passenger has less chances to survive. A 3rd class passenger has even lower chances to survive. Both differences are statistically significant.

Our baseline category in terms of gender is a woman. The model shows, that a male would have a significantly lower chance to survive.

This situation is different for different classes. For example, among the 2nd class passengers the survival chances for men and women are less different. This is even more true for the 1st class passengers.

Let's calculate predictions for a few cases using our model:


 - Survival odds for a woman travelling 1st class: $O = \exp(3.517) = 33.7$  
 This corresponds to survival probability of: $P = 33.7 / (33.7 + 1) = 0.971$

 - Survival odds for a man travelling 1st class: $O = \exp(3.517 - 4.433) = 0.4$   
 Survival probability: $P = 0.4 / 1.4 = 0.286$

 - Survival odds for a man travelling 2nd class: $O = \exp(3.517 - 4.433 - 1.539 + 0.549) = 0.149$   
 Survival probability: $P = 0.149 / 1.149 = 0.13$

- Survival odds for a man travelling 3rd class: $O = \exp(2.955 - 4.433 - 3.582 + 2.786 ) = 0.103$  
Survival probability: $P = 0.103 / 1.103 = 0.0934$
...
predict(m_lr_sexclass, newdata) %>% exp()
```


As mentioned above, the slope coefficients for a logistic regression model show the change in log-odds for a unit change in predictor. Since $\ln a - \ln b = \ln a/b$, we can consider the slope coefficient as log-odds ratio for unit change in a predictor. Zero slope corresponds to no change. Instead of log odds ratio, we'd better compare the odds ratio itself. So we can exponentiate the slope coefficients:
...
Now we can clearly see, that for the baseline category (woman in the 1st class) the survival odds are 33.7, for a man in the same class odds are lower (19.2 * 0.0119 = 0.228) etc. 
